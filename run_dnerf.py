...
class train(LightningModule):
    ...
    def __init__(self):
        ...
        skips = [4]
        self.model = NeRF.get_by_name(self.args.nerf_type, D=self.args.netdepth, W=self.args.netwidth,
                    input_ch=self.input_ch, output_ch=self.output_ch, skips=[4],
                    input_ch_views=self.input_ch_views, input_ch_time=self.input_ch_time,
                    use_viewdirs=self.args.use_viewdirs, embed_fn=self.embed_fn,
                    zero_canonical=not self.args.not_zero_canonical)

        self.model_fine = None
        if self.args.use_two_models_for_fine:
            self.model_fine = NeRF.get_by_name(self.args.nerf_type, D=self.args.netdepth_fine, W=self.args.netwidth_fine,
                            input_ch=self.input_ch, output_ch=self.output_ch, skips=skips,
                            input_ch_views=self.input_ch_views, input_ch_time=self.input_ch_time,
                            use_viewdirs=self.args.use_viewdirs, embed_fn=self.embed_fn,
                            zero_canonical=not self.args.not_zero_canonical)

        self.network_query_fn = lambda inputs, viewdirs, ts, network_fn : run_network(inputs, viewdirs, ts, network_fn,
                                                                    embed_fn=self.embed_fn,
                                                                    embeddirs_fn=self.embeddirs_fn,
                                                                    embedtime_fn=self.embedtime_fn,
                                                                    netchunk=self.args.netchunk,
                                                                    embd_time_discr=self.args.nerf_type!="temporal")
        ...
        self.render_kwargs_train = {
            ...
            'use_two_models_for_fine' : self.args.use_two_models_for_fine,
        }

        # NDC only good for LLFF-style forward facing data
        if self.args.dataset_type != 'llff' or self.args.no_ndc:
            self.render_kwargs_train['ndc'] = False
            self.render_kwargs_train['lindisp'] = self.args.lindisp
    ...
    def configure_optimizer(self):
        ...
        if self.args.do_half_precision:
            ...
            if self.model_fine is not None:
                [self.model, self.model_fine], optimizers = amp.initialize([self.model, self.model_fine], optimizer, opt_level='O1')
            else:
                self.model, optimizers = amp.initialize(self.model, optimizer, opt_level='O1')
        return optimizers

    def checkpoint_callback(self):

        if self.args.ft_path is not None and self.args.ft_path!='None':
            ckpts = ModelCheckpoint(self.ft_path)
        else:
            ckpts = ModelCheckpoint(os.path.join(self.args.basedir, self.args.expname, f) for f in sorted(os.listdir(os.path.join(self.args.basedir, self.args.expname))) if 'tar' in f)
    ...
    def prepare_data(self, args):
        ...
        if args.dataset_type == 'blender':
            ...
            print('Loaded blender', self.images.shape, self.render_poses.shape, self.hwf, args.datadir)
        ...
        self.min_time, self.max_time = self.times[self.i_train[0]], self.times[self.i_train[-1]]
        assert self.min_time == 0., "time must start at 0"
        assert self.max_time == 1., "max time must be 1"
    ...
    def forward(self, args, rays_rgb, i_batch, N_rand):
        ...
        self.render_times = torch.Tensor(self.render_times)
        ...
        for i in trange(start, N_iters):
            ...
            if self.args.use_batching:
                ...
                if i >= args.precrop_iters_time:
                    ...
                    skip_factor = i / float(args.precrop_iters_time) * len(self.i_train)
                    max_sample = max(int(skip_factor), 3)
                    img_i = np.random.choice(self.i_train[:max_sample])

                target = self.images[img_i]
                pose = self.poses[img_i, :3, :4]
                frame_time = self.times[img_i]

                if N_rand is not None:
                    rays_o, rays_d = get_rays(self.H, self.W, self.focal, torch.Tensor(pose))  # (H, W, 3), (H, W, 3)

                    if i < args.precrop_iters:
                        dH = int(self.H//2 * args.precrop_frac)
                        dW = int(self.W//2 * args.precrop_frac)
                        coords = torch.stack(
                            torch.meshgrid(
    ...
    def training_setp(self):
        ...
        if args.render_only:
            ...
            with torch.no_grad():
                ...
                rgbs, _ = render_path(self.render_poses, self.render_times, self.hwf, args.chunk, self.render_kwargs_test, gt_imgs=self.images,
